...
//if($_SERVER["REQUEST_METHOD"] == "POST"){ 
   if(isset($_GET['vKey'])){
     ...
     $resultSet = $con->query("SELECT verified,verifyKey FROM user WHERE verified = 0 AND verifyKey = '$verifyKey' LIMIT 1");

     if($resultSet->num_rows == 1){
       //Validate the email
       $update = $con->query("UPDATE USER SET verified = 1 WHERE verifyKey = '$verifyKey' LIMIT 1");

       if($update){
          $Error = "Your account has been verified.you now can <a href='login.php'>login</a>.";
          $head = "The account is successfully verified.";
       }
       else{
         $Error = $con->error;
       }
...
else if(empty($_GET['vKey'])){
    $Error = "you are fail!";
    $head  = "An uncaught error !";
}
else{
...
<div class="row">
    ...
    <div class="col-1">
        <div class="col-4">
           <h2 class="font2 center big"><?php echo isset($head) ? $head : $_SESSION['head'] ; ?></h2>
            <p class="font1 center"><?php echo isset($Error) ? $Error : $_SESSION['error']; ?></p>
        </div>
        <div class="col-2">
